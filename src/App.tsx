...
function InkDither({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      sizeRef.current = { w, h };
      cols = Math.max(1, Math.round(w / cellSize));
      rows = Math.max(1, Math.round(h / cellSize));
    ...
    function frame(t) {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          const size = Math.max(
            0.6,
            cellSize * 0.85 * Math.min(1, ink01raw * pulseAmt)
          );
...
function Flourish({ className = '' }) {
  return (
    ...
    >
      <path
        d="M0 13 C 26 3 38 23 58 13 C 78 3 90 23 110 13"
        stroke="currentColor"
        strokeWidth="1"
      />
      <circle cx="120" cy="13" r="2.6" fill="currentColor" />
      <path
        d="M130 13 C 150 23 162 3 182 13 C 202 23 214 3 240 13"
        stroke="currentColor"
        strokeWidth="1"
      />
    </svg>
...
const initialClients = [
  {
    ...
    endereco: 'Rua das Acácias, 210 – Atibaia/SP',
    cpf: '321.900.554-10',
    cadastradoEm: '12/03/2025',
    anamnese: {
      alergias: 'Nenhuma alergia conhecida',
      medicamentos: 'Nenhum em uso',
      condicoes: 'Nenhuma condição relevante relatada',
      queloide: 'Não',
      gestanteLactante: 'Não',
      assinadoEm: '12/03/2025',
      validaAte: '12/03/2027',
    },
    blacklist: false,
  },
  {
    id: 2,
    nome: 'Diego Salgado',
    telefone: '(11) 97110-2298',
    endereco: 'Av. Horácio Neto, 880 – Atibaia/SP',
    cpf: '455.221.870-02',
    cadastradoEm: '02/05/2024',
    anamnese: {
      alergias: 'Alergia a esparadrapo comum',
      medicamentos: 'Nenhum em uso',
      condicoes: 'Nenhuma condição relevante relatada',
      queloide: 'Não',
      gestanteLactante: 'Não',
      assinadoEm: '02/05/2024',
      validaAte: '02/05/2026',
    },
    blacklist: true,
    problema:
      'Faltou a 3 sessões seguidas sem aviso prévio e se recusou a pagar o sinal já combinado.',
    blacklistDesde: '18/11/2024',
  },
  {
    id: 3,
    nome: 'Juliana Prado',
    telefone: '(11) 99304-5561',
    endereco: 'Rua Bela Vista, 55 – Atibaia/SP',
    cpf: '198.774.330-45',
    ...
    anamnese: {
      ...
      condicoes: 'Nenhuma condição relevante relatada',
      queloide: 'Sim, histórico leve',
      gestanteLactante: 'Não',
      assinadoEm: '27/01/2026',
      validaAte: '27/01/2028',
    },
    blacklist: false,
  },
  {
    id: 4,
    nome: 'Pedro Anacleto',
    telefone: '(11) 98812-0034',
    endereco: 'Rua Dr. Fábio, 140 – Atibaia/SP',
    cpf: '552.410.667-88',
    ...
    anamnese: {
      ...
      queloide: 'Não',
      gestanteLactante: 'Não',
      assinadoEm: '09/08/2026',
      validaAte: '09/08/2028',
    },
    blacklist: false,
  },
];
...
const initialAppointments = [
  {
    ...
    hora: '10:00',
    duracao: 2,
    servico: 'Fechamento de braço – blackwork',
    status: 'agendado',
  },
  {
    id: 2,
    ...
    hora: '14:00',
    duracao: 1.5,
    servico: 'Botânico fineline – antebraço',
    status: 'agendado',
  },
  {
    id: 3,
    ...
    hora: '09:30',
    duracao: 3,
    servico: 'Realismo – retrato costas',
    status: 'agendado',
  },
  {
    id: 4,
  ...
  },
  {
    ...
    hora: '13:00',
    duracao: 2,
    servico: 'Sessão 2 – fechamento braço',
    status: 'agendado',
  },
];
...
const initialProdutos = [
  {
    id: 1,
    nome: 'Tinta preta Eternal 30ml',
    quantidade: 12,
    valorPago: 68,
    valorVenda: 0,
    dataCompra: '03/08/2026',
    fornecedor: 'Ink Supply BR',
  },
  {
    id: 2,
    nome: 'Agulha RL 09 (cx 50un)',
    quantidade: 4,
    valorPago: 95,
    valorVenda: 0,
    dataCompra: '20/07/2026',
    fornecedor: 'TattooMED',
  },
  {
    id: 3,
    nome: 'Pomada cicatrizante 60g',
    quantidade: 22,
    valorPago: 14,
    valorVenda: 35,
    dataCompra: '10/08/2026',
    fornecedor: 'Derma Cuidados',
  },
  {
    id: 4,
    nome: 'Filme PVC protetor (rolo)',
    quantidade: 3,
    valorPago: 42,
    valorVenda: 0,
    dataCompra: '15/06/2026',
    fornecedor: 'Ink Supply BR',
  },
  {
    id: 5,
    nome: 'Camiseta Arte Viva P/M/G',
    quantidade: 18,
    valorPago: 28,
    valorVenda: 69,
    dataCompra: '01/08/2026',
    fornecedor: 'Estamparia Bento',
  },
];
...
const templatesMensagem = [
  {
    id: '24h',
    titulo: '24 horas antes',
    ativo: true,
    texto:
      'Oi {{nome}}! Passando para confirmar sua sessão amanhã, {{data}} às {{hora}}, com {{artista}} no Arte Viva. Chegue com 10 minutos de antecedência e evite álcool no dia anterior. Qualquer coisa é só responder por aqui.',
  },
  {
    id: '1h',
    titulo: '1 hora antes',
    ativo: true,
    texto:
      '{{nome}}, sua sessão com {{artista}} começa em 1 hora aqui no Arte Viva. Te esperamos!',
  },
  {
    id: 'final',
    titulo: 'Finalização do trabalho',
    ativo: true,
    texto:
      'Prontinho, {{nome}}! Sua tattoo foi finalizada hoje. Cuidados: mantenha a proteção por 2h, lave com sabão neutro 2x ao dia, use a pomada indicada e evite sol e piscina por 15 dias. Vamos te chamar em {{diasRetorno}} dias para avaliar a cicatrização.',
  },
];
...
function Login({ onEntrar, artCanvas }) {
  return (
    <div
      className="av-root min-h-screen flex items-center justify-center p-6"
    ...
    >
      ...
      <div className="av-content-layer w-full max-w-3xl">
        <div className="text-center mb-10">
          <div className="av-wordmark text-6xl font-bold uppercase">
            Arte Viva
          </div>
          <Flourish className="w-56 h-6 mx-auto av-faded mt-4 mb-3" />
          <div className="av-faded text-sm tracking-wide">
            estúdio de tatuagem · sistema de gestão
          </div>
        </div>
        ...
        <div className="av-faded text-xs text-center mt-8">
          Cada tatuador acessa apenas sua própria agenda e seus próprios
          atendimentos.
        </div>
      </div>
    </div>
...
function Shell({ artista, tab, setTab, onSair, children, artCanvas }) {
  return (
    <div className="av-root min-h-screen flex">
      <aside
        className="w-60 av-surface border-r av-hair flex flex-col shrink-0"
        style={{ position: 'relative' }}
      >
        <div className="av-ink-layer">
          {artCanvas && (
            <InkDither srcCanvas={artCanvas} opacity={0.1} cellSize={8} />
          )}
        </div>
        <div className="av-content-layer px-6 py-6 av-scar">
          <div className="av-wordmark av-accent text-xl font-bold uppercase leading-none">
            Arte Viva
          </div>
          <Flourish className="w-24 h-3 av-faded mt-2 mb-1" />
          <div className="av-faded text-[11px]">sistema de gestão</div>
        </div>
        <nav className="av-content-layer flex-1 py-3">
          {NAV.map(({ id, label, Icon }) => (
            <button
              key={id}
              onClick={() => setTab(id)}
              className={`av-nav-item w-full flex items-center gap-3 px-6 py-3 text-sm text-left ${
                tab === id ? 'av-nav-active text-[#F5F5F2]' : 'av-faded'
              }`}
            >
        ...
        <div className="av-content-layer px-6 py-5 av-hair border-t">
          ...
          <div className="av-faded text-xs mb-3">
            {ARTISTS[artista].especialidade}
          </div>
          <button
            onClick={onSair}
            className="av-faded text-xs flex items-center gap-1.5 hover:text-[#F5F5F2]"
          >
...
function PageHeader({ title, sub, right }) {
  return (
    <div className="flex items-start justify-between px-8 py-7 av-hair border-b">
      <div>
...
function Agenda({ artista, appointments, setAppointments, clients }) {
  ...
  const visiveis = appointments.filter(
    (a) =>
      a.status !== 'concluido' && (filtro === 'todos' || a.artista === artista)
  );
  const cliente = (id) => clients.find((c) => c.id === id);

  function finalizar(ap) {
    setAppointments((prev) =>
      prev.map((a) => (a.id === ap.id ? { ...a, status: 'concluido' } : a))
    );
    setAviso(
      `Mensagem de finalização enviada por WhatsApp para ${
        cliente(ap.clienteId).nome
      }: cuidados pós-tattoo e retorno de avaliação em 15 dias.`
    );
    setSelecionado(null);
  ...
  return (
    <div>
        ...
        right={
          <div className="flex av-hair border rounded-sm overflow-hidden text-xs">
            <button
              ...
              className={`px-4 py-2 ${
                filtro === 'meus' ? 'av-btn-primary' : 'av-faded'
              }`}
            >
              Meus agendamentos
            </button>
            <button
              ...
              className={`px-4 py-2 ${
                filtro === 'todos' ? 'av-btn-primary' : 'av-faded'
              }`}
            >
              Todos os artistas
            </button>
      ...
      <div className="p-8 grid grid-cols-6 gap-3">
        {DIAS.map((dia, i) => (
          <div key={dia} className="min-h-[420px]">
            <div className="av-faded text-xs mb-2 pb-2 av-hair border-b">
              {dia}
            </div>
              ...
              {visiveis
                ...
                .map((a) => {
                  ...
                  return (
                    ...
                    >
                      <div className="text-[11px] av-faded flex items-center gap-1">
                        <Clock size={10} /> {a.hora} · {isAlex ? 'A' : 'C'}
                      </div>
                      <div className="text-xs font-medium mt-1 leading-snug">
                        {c.nome}
                      </div>
                      <div className="av-faded text-[11px] mt-0.5 leading-snug">
                        {a.servico}
                      </div>
                    </button>
                  );
      ...
      {selecionado && (
        <Modal onClose={() => setSelecionado(null)}>
          <div className="av-display text-xl font-bold mb-1">
            {cliente(selecionado.clienteId).nome}
          </div>
          <div className="av-faded text-sm mb-5">{selecionado.servico}</div>
          <div className="grid grid-cols-2 gap-4 text-sm mb-6">
            <Info
              label="Data"
              value={`${DIAS[selecionado.dia]} · ${selecionado.hora}`}
            />
            <Info label="Duração prevista" value={`${selecionado.duracao} h`} />
            <Info label="Artista" value={ARTISTS[selecionado.artista].nome} />
            <Info
              label="Telefone"
              value={cliente(selecionado.clienteId).telefone}
            />
          </div>
          <div className="flex gap-3">
            <button
              onClick={() => finalizar(selecionado)}
              className="av-btn-primary rounded-full px-4 py-2.5 text-sm font-medium flex items-center gap-2"
            >
              <Check size={15} /> Finalizar atendimento
            </button>
            <button
              onClick={() => setSelecionado(null)}
              className="av-btn-ghost rounded-full px-4 py-2.5 text-sm"
            >
              Fechar
            </button>
          </div>
          <div className="av-faded text-xs mt-4">
            Ao finalizar, o horário sai da agenda e o cliente recebe a mensagem
            de cuidados com data de retorno para avaliação.
          </div>
        </Modal>
      )}
    </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-6 z-50">
      <div className="av-card rounded-sm p-7 max-w-md w-full relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 av-faded hover:text-[#F5F5F2]"
        >
          <X size={18} />
        </button>
        {children}
...
function Clientes({ clients, setClients }) {
  ...
  function marcarBlacklist() {
    setClients((prev) =>
      prev.map((x) =>
        x.id === c.id
          ? { ...x, blacklist: true, problema, blacklistDesde: '31/08/2026' }
          : x
      )
    );
    setMarcando(false);
  ...
  const filtrados = clients.filter((x) =>
    ...
    <div>
        ...
        <div className="av-hair border-r min-h-[600px]">
          <div className="p-4 av-hair border-b">
            <div className="relative">
              <Search
                size={14}
                className="absolute left-3 top-1/2 -translate-y-1/2 av-faded"
              />
              <input
                value={busca}
                onChange={(e) => setBusca(e.target.value)}
                placeholder="Buscar cliente"
                className="av-input rounded-sm w-full pl-8 pr-3 py-2 text-sm"
              />
            </div>
          </div>
          {filtrados.map((x) => (
            <button
              ...
              className={`w-full text-left px-4 py-3 av-hair border-b av-nav-item ${
                selId === x.id ? 'av-surface-2' : ''
              }`}
            >
        ...
        <div className="p-8">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <div className="av-faded text-sm">
                Cliente desde {c.cadastradoEm}
              </div>
            </div>
            {!c.blacklist && (
              <button
                onClick={() => setMarcando(true)}
                className="av-btn-ghost rounded-sm px-3 py-2 text-xs flex items-center gap-1.5"
              >
          ...
          <div className="grid grid-cols-2 gap-4 mb-8 text-sm">
            ...
            <Info
              label="Situação"
              value={c.blacklist ? 'Lista negra' : 'Regular'}
            />
          </div>
          ...
          {c.blacklist && (
            <div className="av-card av-warn rounded-sm p-4 mb-8 text-sm">
              <div className="font-medium mb-1">
                Ocorrência registrada em {c.blacklistDesde}
              </div>
          ...
          <div className="av-paper av-scar rounded-sm p-6 relative overflow-hidden">
            ...
            <div className="relative">
              <div className="av-display text-lg font-bold mb-0.5">
                Ficha de anamnese
              </div>
              <div className="text-xs mb-5" style={{ color: '#6B6B66' }}>
                Gerada automaticamente a partir do cadastro · assinada em{' '}
                {c.anamnese.assinadoEm} · guardada até {c.anamnese.validaAte}{' '}
                conforme LGPD
              </div>
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <PaperField
                  ...
                  value={c.anamnese.medicamentos}
                />
                <PaperField
                  label="Condições de saúde"
                  value={c.anamnese.condicoes}
                />
                <PaperField
                  label="Histórico de queloide"
                  value={c.anamnese.queloide}
                />
                <PaperField
                  label="Gestante ou lactante"
                  value={c.anamnese.gestanteLactante}
                />
              </div>
            </div>
          </div>
      ...
      {marcando && (
        <Modal onClose={() => setMarcando(false)}>
          <div className="av-display text-xl font-bold mb-4">
            Marcar {c.nome} na lista negra
          </div>
          ...
          <textarea
            ...
            rows={4}
            className="av-input rounded-sm w-full p-3 text-sm mb-5"
            placeholder="Ex.: cliente causou dano à sala, deixou de pagar, etc."
          />
          <div className="flex gap-3">
            <button
              onClick={marcarBlacklist}
              disabled={!problema}
              className="av-btn-primary rounded-full px-4 py-2.5 text-sm font-medium disabled:opacity-40"
            >
              Confirmar
            </button>
            <button
...
function PaperField({ label, value }) {
  return (
    <div>
      <div className="text-[11px] mb-1" style={{ color: '#7A7A74' }}>
        {label}
      </div>
      <div>{value}</div>
    </div>
        ...
        <div className="p-8 grid grid-cols-2 gap-3">
          {vendaveis.map((p) => (
            <button
              key={p.id}
              onClick={() => add(p)}
              className="av-card rounded-sm p-4 text-left hover:av-surface-2"
            >
              <div className="text-sm font-medium">{p.nome}</div>
              <div className="text-sm mt-2">{formatBRL(p.valorVenda)}</div>
              <div className="av-faded text-[11px] mt-1">
        ...
        <div className="av-hair border-l p-6 flex flex-col min-h-[500px]">
              ...
              >
                <div>
                  ...
                  <div className="av-faded text-xs">
                    {i.qtd} × {formatBRL(i.valorVenda)}
                  </div>
                </div>
                <button
                  onClick={() => remover(i.id)}
                  className="av-faded hover:text-[#F5F5F2]"
                >
                  <Trash2 size={14} />
                </button>
              </div>
          ...
          <div className="av-hair border-t pt-4 mt-4">
            ...
            <div className="flex gap-2 mb-4">
              {['pix', 'cartão', 'dinheiro'].map((f) => (
                <button
                  ...
                  className={`flex-1 rounded-sm py-2 text-xs capitalize ${
                    pagamento === f ? 'av-btn-primary' : 'av-btn-ghost'
                  }`}
                >
                  {f}
                </button>
            ...
            <button
              ...
              className="av-btn-primary rounded-full w-full py-2.5 text-sm font-medium disabled:opacity-40"
            >
    ...
    <div>
              ...
              <tr key={p.id} className="av-hair border-b">
                ...
                <td className="py-3">
                  {p.valorVenda ? (
                    ...
                    <span className="av-faded">uso interno</span>
                  )}
                </td>
      ...
      {novo && (
        <Modal onClose={() => setNovo(false)}>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <LabeledInput
                ...
                onChange={(v) => setForm({ ...form, valorPago: v })}
              />
              <LabeledInput
                label="Valor de venda"
                value={form.valorVenda}
                onChange={(v) => setForm({ ...form, valorVenda: v })}
              />
              <LabeledInput
                label="Data da última compra"
                value={form.dataCompra}
                onChange={(v) => setForm({ ...form, dataCompra: v })}
              />
            </div>
            <LabeledInput
          ...
          <button
            onClick={salvar}
            className="av-btn-primary rounded-full px-4 py-2.5 text-sm font-medium mt-5"
          >
            Salvar produto
          </button>
        </Modal>
  ...
  return (
    <div>
      ...
      <input
        ...
        className="av-input rounded-sm w-full px-3 py-2 text-sm"
      />
    </div>
  );
}
...
function Blacklist({ clients }) {
  ...
  return (
    <div>
      <PageHeader
        title="Lista negra"
        sub="Clientes com ocorrência registrada"
      />
      <div className="p-8 space-y-4">
        {marcados.length === 0 && (
          <div className="av-faded text-sm">Nenhum cliente na lista negra.</div>
        )}
        {marcados.map((c) => (
          <div key={c.id} className="av-card av-warn rounded-sm p-5">
            ...
            <div className="av-faded text-xs mt-1 mb-3">
              {c.telefone} · {c.cpf}
            </div>
...
function Relatorios() {
  return (
    <div>
      <PageHeader
        title="Relatórios"
        sub="Visão geral do estúdio · agosto de 2026"
      />
      <div className="p-8">
        ...
        <div className="av-card rounded-sm p-6">
          <div className="av-display font-bold mb-4">
  ...
  return (
    <div>
      <PageHeader
        ...
        sub="Envios por WhatsApp integrados à agenda"
      />
      <div className="p-8 space-y-4 max-w-2xl">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                onClick={() =>
                  setTemplates((prev) =>
                    prev.map((x) =>
                      x.id === t.id ? { ...x, ativo: !x.ativo } : x
                    )
                  )
                }
                className={`w-10 h-5 rounded-full relative transition-colors ${
                  t.ativo ? 'av-btn-primary' : 'av-hair border'
                }`}
              >
                <span
                  className={`absolute top-0.5 w-4 h-4 rounded-full transition-all ${
                    t.ativo ? 'left-5 bg-black' : 'left-0.5 bg-[#F5F5F2]'
                  }`}
                />
              </button>
            </div>
        ...
        <div className="av-faded text-xs">
          Os campos entre chaves são preenchidos automaticamente com os dados do
          agendamento e do cadastro do cliente.
        </div>
      </div>
    </div>
  );
